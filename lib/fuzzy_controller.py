...
class Cffuzzyc:
    ...
    def auto(self, rfront=0, rback=0, rright=0, rleft=0, rup=0, rzrange=0):
        ...
        rules1 = Rule( {(self._front_range.far, self._rear_range.far): self._y_vel.forward,
        ...
        rules2 = Rule({(self._right_range.close, self._left_range.far): self._x_vel.left,
                        (self._left_range.close, self._left_range.far): self._x_vel.right,
                        })
            
        rg1 = {self._front_range : block_saturation(rfront), self._rear_range : block_saturation(rback)
            }
        rg2 = {self._left_range : block_saturation(rleft), self._right_range : block_saturation(rright)
            }
